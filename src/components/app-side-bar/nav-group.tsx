...
import { Box, BoxProps, Button, Collapse, Heading, Spacer, useColorMode, VStack } from "@chakra-ui/react";
...
const NavGroup: React.FC<NavGroupProps> = props => {
    let { title, open: openProp, onOpen, openable, children, ...restProps } = props;
    ...
    let headingColor = colorMode === 'light' ? 'blackAlpha.700' : 'whiteAlpha.700';
    let iconColor = headingColor;

    if (title){
        heading = <Heading as='h4' color={headingColor} size='xs' textTransform='uppercase'>{title}</Heading>
    }

    let chevronIcon = isOpen ?  <ChevronUpIcon color={iconColor} /> :
